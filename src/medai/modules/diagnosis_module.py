...
class DiagnosticModule:
    ...
    def __init__(self, 
        ...
        """
        Initialize the DiagnosticModule.
        
        Args:
            class_names: List of class names.
            model: Pre-loaded model instance. If None, tries to load from checkpoint_path.
            checkpoint_path: Path to model checkpoint. Required if model is None.
            model_name: Name of model architecture (e.g. 'swin'). Required if model is None.
        ...
        if model is not None:
            ...
            self.model.eval()
        elif checkpoint_path and model_name:
            ...
            try:
                ...
                self.model.eval()
                print(f"✅ Diagnostic Module loaded model from {checkpoint_path} on {self.device}.")
            ...
            raise ValueError("Either 'model' OR ('checkpoint_path' AND 'model_name') must be provided.")

        # 3. Setup Transforms
        self.transform = get_transforms(self.img_size)

    def run_diagnosis(self, image_input: Union[str, Image.Image]) -> Dict[str, Any]:
        """
        ...
        # Prepare probabilities dictionary with swapped labels (matching app.py logic)
        all_probs_dict = {}
        for i in range(len(probs_np)):
            class_name = self.class_names[i]
            swapped_name = _swap_prediction_label(class_name)
            all_probs_dict[swapped_name] = float(probs_np[i])
        
        # Keep list format for legacy compatibility if needed
        # Swap indices in the list array to match the label swap
        try:
            # Find indices safely
        ...
        result = {
            ...
            "all_probabilities": probs_np.tolist(), # List format (swapped)
            "all_probabilities_dict": all_probs_dict # Dict format (swapped keys)
        }
    ...
    # Alias for compatibility with app.py which uses .diagnose()
    def diagnose(self, image_input: Union[str, Image.Image]) -> Dict[str, Any]:
        return self.run_diagnosis(image_input)

# --- 3. EXECUTION ---

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Run a diagnostic module on a single image.')
    ...
    # Run the Diagnosis
    result = module.run_diagnosis(args.image_path)
